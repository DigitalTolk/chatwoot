class Api::V1::Widget::BaseController < ApplicationController
  ...
  def conversations
    if @contact_inbox.hmac_verified?
      ...
      @conversations = @contact.conversations.unarchived.where(contact_inbox_id: verified_contact_inbox_ids)
    else
      @conversations = @contact_inbox.conversations.unarchived.where(inbox_id: auth_token_params[:inbox_id])
    end
  end
