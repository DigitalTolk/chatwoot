  ...
  <div>
    ...
    >
      ...
      <div v-if="filteredSmartActions.length" class="mt-8">
        ...
        >
          {{ filteredSmartActions.length }} smart actions detected from
          conversation
        </h1>
        <p class="text-slate-400 dark:text-slate-300">
          Proceed with each action and let AI do the rest e.g auto fill form
        </p>
      </div>
      <div v-else>
        <p class="text-slate-400 dark:text-slate-300">
          No smart actions found.
        </p>
      </div>
      <div class="mt-3 action-holder">
        <div
          v-for="action in filteredSmartActions"
          :key="action.index"
          class="smart-action-item bg-slate-25 dark:bg-slate-900 m-0 h-full min-h-0"
        >
          <div
            class="capitalize float-right rounded-full bg-green-100 text-xs px-3 py-2 dark:text-slate-300 text-slate-700"
          >
            {{ action.intent_type }}
          </div>
          <h1
            class="text-black-900 dark:text-slate-400 text-2xl font-medum mb-2"
          >
            {{ action.name }}
          </h1>
          <p class="text-slate-400 dark:text-slate-300 mb-3">
            {{ action.description }}
          </p>
      ...
      <div class="absolute bottom-0 w-11/12">
        ...
        />
        <div
          class="flex smart-help-chevron text-slate-400 dark:text-slate-300 text-xs py-2"
          @click="toggleHelp"
        >
          <fluent-icon
            v-if="showHelp"
        ...
        <p v-if="showHelp" class="text-sm text-slate-800 dark:text-slate-300">
          Smart action is an AI-powered feature designed to assist support
          agents in real-time conversation analysis. With this new tool, agents
          can seamlessly detect key actions and cues during ongoing
          conversations, empowering them to provide faster, more accurate
          assistance to customers.
        </p>
      </div>
    </div>
    <div
      v-if="showBookingPanel"
